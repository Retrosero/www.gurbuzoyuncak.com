-- Migration: advanced_product_detail_new_tables
-- Created at: 1761980356

-- Gelişmiş Ürün Detay Sayfası için Yeni Database Tabloları
...
CREATE INDEX IF NOT EXISTS idx_product_specifications_spec_group ON product_specifications(spec_group);
...
CREATE INDEX IF NOT EXISTS idx_related_products_relation_type ON related_products(relation_type);
...
CREATE INDEX IF NOT EXISTS idx_product_price_history_valid_from ON product_price_history(valid_from);
...
CREATE INDEX IF NOT EXISTS idx_product_stock_movements_type ON product_stock_movements(movement_type);
...
DROP POLICY IF EXISTS "Users can view their own product views" ON user_product_views;
CREATE POLICY "Users can view their own product views" ON user_product_views
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own product views" ON user_product_views;
CREATE POLICY "Users can insert their own product views" ON user_product_views
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Admin policies for analytics
DROP POLICY IF EXISTS "Admins can view all product analytics" ON product_analytics;

-- Migration: user_favorites_complete_system
-- Created at: 1761979138
...
SELECT 
    ...
    fph.change_amount as last_price_change_amount,
...
-- Yorumlar
COMMENT ON TABLE favorite_price_history IS 'Favori ürünlerin fiyat değişim geçmişi ve bildirim takibi';
COMMENT ON TABLE favorite_stock_alerts IS 'Favori ürünlerin stok değişim uyarıları';
COMMENT ON TABLE favorite_notification_settings IS 'Kullanıcıların favori bildirim tercihleri';
COMMENT ON VIEW user_favorites_detailed IS 'Kullanıcı favorilerinin detaylı görünümü - ürün bilgileri, son değişiklikler, istatistikler';
COMMENT ON VIEW favorite_price_tracking IS 'Favori ürünlerin fiyat takip görünümü - değişim analizi ve bildirim durumu';
COMMENT ON VIEW favorite_stock_tracking IS 'Favori ürünlerin stok takip görünümü - stok seviyesi ve uyarı durumu';

-- Trigger fonksiyonlarının yorumları
COMMENT ON FUNCTION set_favorite_price_threshold() IS 'Yeni favori eklendiğinde fiyat değişim eşiğini varsayılan değere ayarlar';
COMMENT ON FUNCTION detect_price_change(INTEGER, DECIMAL, DECIMAL) IS 'Ürün fiyat değişimini tespit eder ve anlamlı değişiklikleri belirler';
COMMENT ON FUNCTION detect_stock_change(INTEGER, INTEGER, INTEGER) IS 'Ürün stok değişimini tespit eder ve uyarı tipini belirler';;
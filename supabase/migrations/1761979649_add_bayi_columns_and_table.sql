-- Migration: add_bayi_columns_and_table
-- Created at: 1761979649
...
CREATE POLICY IF NOT EXISTS "Kullanıcılar kendi bakiye geçmişlerini görebilir"
ON balance_transactions FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

-- Bayiler sadece kendi siparişlerini görebilir
CREATE POLICY IF NOT EXISTS "Bayiler sadece kendi siparişlerini görebilir"
ON bayi_orders FOR SELECT
TO authenticated
USING (auth.uid() = bayi_id);

-- Servis rolü tüm işlemleri yapabilir
CREATE POLICY IF NOT EXISTS "Servis rolü tüm işlemleri yapabilir - balance"
ON balance_transactions FOR ALL
TO service_role
USING (true);

CREATE POLICY IF NOT EXISTS "Servis rolü tüm işlemleri yapabilir - bayi_orders"

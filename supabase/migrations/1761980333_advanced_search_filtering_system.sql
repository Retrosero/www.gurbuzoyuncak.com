-- Migration: advanced_search_filtering_system
-- Created at: 1761980333
...
USING gin(to_tsvector('turkish', name || ' ' || COALESCE(description, '') || ' ' || COALESCE(tags::text, '')));
...
BEGIN
    -- Base search query
    search_query := 'to_tsvector(''turkish'', name || '' '' || COALESCE(description, '''') || '' '' || COALESCE(tags::text, '''')) @@ plainto_tsquery(''turkish'', $1)';
    ...
    IF p_min_age IS NOT NULL THEN
        age_filter := age_filter || ' AND min_age >= $6';
    END IF;
    IF p_max_age IS NOT NULL THEN
        age_filter := age_filter || ' AND max_age <= $7';
    END IF;
    ...
    -- Sort clause
    CASE p_sort_by
        WHEN 'relevance' THEN sort_clause := ' ORDER BY ts_rank(to_tsvector(''turkish'', name || '' '' || COALESCE(description, '''') || '' '' || COALESCE(tags::text, '''')), plainto_tsquery(''turkish'', $1)) DESC';
        ...
        ELSE sort_clause := ' ORDER BY ts_rank(to_tsvector(''turkish'', name || '' '' || COALESCE(description, '''') || '' '' || COALESCE(tags::text, '''')), plainto_tsquery(''turkish'', $1)) DESC';
    END CASE;
    ...
    RETURN QUERY EXECUTE '
        SELECT 
            ...
            p.product_code,
            p.base_price,
            COALESCE(p.sale_price, p.base_price) as sale_price,
            b.name as brand_name,
            c.name as category_name,
            p.stock,
            p.is_featured,
            p.image_url,
            ts_rank(to_tsvector(''turkish'', p.name || '' '' || COALESCE(p.description, '''') || '' '' || COALESCE(p.tags::text, '''')), plainto_tsquery(''turkish'', $1)) as search_rank,
        ...
        ' || sort_clause || '
        LIMIT $8 OFFSET $9'
    USING p_query, p_category_ids, p_brand_ids, p_min_price, p_max_price, p_min_age, p_max_age, p_limit, p_offset;
END;
$$ LANGUAGE plpgsql;
...
-- Search analytics güncelleme fonksiyonu
CREATE OR REPLACE FUNCTION update_search_analytics(p_date DATE DEFAULT CURRENT_DATE)
RETURNS VOID AS $$
BEGIN
    ...
    SELECT 
        ...
        COUNT(DISTINCT COALESCE(user_id, session_id)) as unique_searchers,
        ...
        COALESCE(
            ...
            ) FILTER (WHERE query_text IS NOT NULL), 
            '[]'::jsonb
    ...
    ON CONFLICT (date) DO UPDATE SET
        ...
        top_search_queries = EXCLUDED.top_search_queries,
        created_at = NOW();
END;
$$ LANGUAGE plpgsql;

-- Trigger: Search query kaydında analytics güncelle
CREATE OR REPLACE FUNCTION trigger_update_search_analytics()
RETURNS TRIGGER AS $$
BEGIN
    -- Background job olarak analytics güncelle
    PERFORM update_search_analytics(DATE(NEW.created_at));
    
    -- Popüler arama terimlerini güncelle
    INSERT INTO popular_searches (query_text, search_count, category)
    VALUES (NEW.query_text, 1, COALESCE(NEW.filters_applied->>'category', NULL))
    ON CONFLICT (query_text, date_range, category)
    DO UPDATE SET 
...
CREATE TRIGGER trigger_update_search_analytics_after_insert
    AFTER INSERT ON search_queries
    FOR EACH ROW
    EXECUTE FUNCTION trigger_update_search_analytics();
...
COMMENT ON FUNCTION update_search_analytics IS 'Günlük arama analitiği verilerini günceller';;
Deno.serve(async (req) => {
    ...
    // =====================================================
    // AKILLI ÜRÜN GÜNCELLEME SİSTEMİ YARDIMCI FONKSİYONLARI
    // =====================================================

    // İki ürün arasındaki farkları hesapla
    function calculateProductDiff(oldProduct: any, newProduct: any): {
    ...
    async function smartUpdateProduct(
        ...
        const memoryUsage = Math.round(performance.memory?.usedJSHeapSize / 1024 / 1024) || 0;
    ...
    // Cancellation endpoint - İyileştirilmiş cancellation sistemi
    ...
    try {
        ...
        async function updateProgress(stage, current, total, message = '') {
            ...
            await fetch(`${supabaseUrl}/rest/v1/xml_imports?id=eq.${importId}`, {
                ...
                body: JSON.stringify({
                    ...
                    memory_usage: progressData.memoryUsage,
        ...
        function parseXML(xml) {
            ...
            for (const match of matches) {
                ...
                // Progress güncelle (her 10 üründe bir)
        ...
        for (let i = 0; i < parsedProducts.length; i += BATCH_SIZE) {
            ...
            // Batch başlangıç progress güncelle
            ...
            try {
                // Batch içindeki ürünleri işle
                for (const product of batch) {
                    try {
                        processingCount++;
                        
                        // Memory usage kontrolü
                        const currentMemoryMB = Math.round(performance.memory?.usedJSHeapSize / 1024 / 1024) || 0;
                        ...
                        // Ürünün var olup olmadığını kontrol et (hem product_code hem barcode ile)
                        let existingProductResponse;
                        try {
                            // Önce product_code ile ara
                            ...
                            // Eğer product_code bulunamazsa barcode ile ara
                            existingProductResponse = await fetch(
                        ...
                        // Görselleri ekle (sadece yeni ürün için veya değişiklik varsa)
                        if (productId) {
                ...
                // Batch tamamlandı - Performans metriklerini kaydet
                ...
                // Batch sonrası progress güncelle
                if (failed_count > 0) {
        ...
        // Final toplamları hesapla
        const finalSuccessCount = imported_count + updated_count;
        
        // Import kaydını güncelle - Final durum

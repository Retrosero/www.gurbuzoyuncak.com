...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["video/mp4", "video/webm", "video/ogg"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for videos" ON storage.objects FOR SELECT USING (bucket_id = 'videos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for videos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'videos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for videos" ON storage.objects FOR UPDATE USING (bucket_id = 'videos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for videos" ON storage.objects FOR DELETE USING (bucket_id = 'videos');`
    ];

...
export default function BrandsPage() {
  ...
  const uploadLogo = async (file: File): Promise<string | null> => {
    try {
      ...
      const fileName = `logo-${Date.now()}-${Math.random().toString(36).substring(2, 15)}.${fileExt}`
      ...
      if (!urlData?.publicUrl) {
        throw new Error('Logo URL oluşturulamadı')
      }
      ...
      if (error.message?.includes('RLS')) {
        throw new Error('Güvenlik politikası hatası: Logo yükleme yetkiniz bulunmuyor.')
      } else if (error.message?.includes('Duplicate')) {
        throw new Error('Bu logo zaten yüklü. Lütfen farklı bir dosya deneyin.')
      } else {
        throw new Error(`Logo yükleme hatası: ${error.message || 'Bilinmeyen hata'}`)
      }
    }
  }
